...
## Non-Model Lettering (February 2026)

A major shift: **lettering is no longer generated by the image model**. Panels are generated as art-only (no text, bubbles, or captions), and a separate script overlays dialogue and narrative from the scene markdown. This keeps wording under version control and avoids model hallucinations or inconsistent lettering.

### Why Post-Production Lettering?

- The model was instructed not to draw text, but sometimes still did; wording could be wrong or truncated.
- Scene prose and dialogue live in markdown; using that source for lettering guarantees consistency and editability.
- Dialogue can be drawn in speech bubbles and narrative in caption boxes with a single, predictable style (e.g. from `definitions.json` palette).

### `overlay_storyboard_text.py`

The overlay script:

1. **Uses the same chunking as the generator** (`divide_scene_into_storyboards`) so each lettered image matches one storyboard image.
2. **Extracts content from scene text**: splits on double-quoted dialogue, keeps narrative sentences (length/period/capitalization filters), and **drops speech-attribution-only lines** (e.g. “Jed whispered, though Sarah was already behind him.”) so only the actual line and real narrative remain.
3. **Splits dialogue vs narrative** per panel: dialogue in bubbles (light fill, dark text), narrative in captions (dark fill, light text). When a panel has both, the bubble is drawn at the top and the caption at the bottom.
4. **Merges content across panels**: all extracted items (up to 24 per chunk) are distributed over the 4 panels instead of dropping everything after the first four, so more of the scene appears.
5. **Optional OCR**: if pytesseract/Tesseract is available, the script detects and covers existing text in each panel before drawing, so only the overlay lettering is visible.

Panel layout rules (e.g. first panel caption at top, second panel narrative at top when alone, second panel left-aligned box with offset) are encoded so lettering fits the 2×2 layout without overlapping key art.

### Generator Changes for Art-Only Panels

To support this workflow, `generate_storyboards.py` was updated:

- **No lettering in prompts**: prompts explicitly tell the model not to draw any text, bubbles, or captions (lettering added in post).
- **Reference images actually used**: `responseModalities` was set to `["TEXT", "IMAGE"]` (not `["IMAGE"]` alone) so the model processes both the prompt and the reference image blobs. Parts order is prompt first, then images, matching the API’s expected format.
- **Explicit INPUT IMAGES section**: the prompt lists each attached image by index and “USE FOR” (character, setting, style, previous panel for continuity), so the model knows what each blob represents.
- **Continuity reference**: for chunks after the first, the previous chunk’s generated image is prepended to the reference list so the next panel continues the same characters, environment, and style.
- **Reference caps**: more character/setting refs per request (e.g. up to 5 character refs, 2 setting refs) so key actors and locations are present when needed.

Together, these changes make the generator output art-only panels that the overlay script can then letter from the scene markdown in a reproducible way.
